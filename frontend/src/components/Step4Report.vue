    ...
    <div class="main-split-layout">
            ...
            >
              ...
              <div class="section-body" v-show="!collapsedSections.has(idx)">
                ...
                <div v-else-if="currentSectionIndex === idx + 1" class="loading-state">
                  ...
                  <span class="loading-text">Se generează {{ section.title }}...</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <!-- Next Step Button - Afișat după finalizare -->
          <button v-if="isComplete" class="next-step-btn" @click="goToInteraction">
            <span>Intră în interacțiune avansată</span>
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (Conținut generat, dar întregul capitol poate să nu fie finalizat) -->
                  <template v-if="log.action === 'section_content'">
                    ...
                    <!-- Section Complete (Capitol generat complet) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- Când este răspunsul final, afișează indicație specială -->
...
const toggleRawResult = (timestamp, event) => {
  // Salvează poziția butonului relativ la viewport
  ...
  // Așteaptă actualizarea DOM, apoi ajustează poziția de scroll pentru a menține butonul în aceeași poziție
...
const toggleSectionCollapse = (idx) => {
  // Doar capitolele finalizate pot fi pliate
  if (!generatedSections.value[idx + 1]) return
...
const toolConfig = {
  'insight_forge': {
    name: 'Deep Insight',
    color: 'purple',
    icon: 'lightbulb' // Icon bec - reprezintă insight
  },
  'panorama_search': {
    name: 'Panorama Search',
    color: 'blue',
    icon: 'globe' // Icon glob - reprezintă căutare panoramică
  },
  'interview_agents': {
    name: 'Agent Interview',
    color: 'green',
    icon: 'users' // Icon utilizatori - reprezintă conversație
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // Icon fulger - reprezintă viteză
  },
  'get_graph_statistics': {
    name: 'Graph Stats',
    color: 'cyan',
    icon: 'chart' // Icon grafic - reprezintă statistici
  },
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // Icon bază de date - reprezintă entități
  }
}
...
const parseInsightForge = (text) => {
  ...
  try {
    // Extrage problema de analiză
    const queryMatch = text.match(/Analiză问题:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Extrage scenariul de predicție
    const reqMatch = text.match(/预测场景:\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()
    
    // Extrage statisticile - potrivire format "fapte de predicție relevante: X"
    const factMatch = text.match(/相关预测事实:\s*(\d+)/)
    const entityMatch = text.match(/涉及Entitate:\s*(\d+)/)
    const relMatch = text.match(/Relație链:\s*(\d+)/)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])
    
    // Extrage sub-întrebările - extragere completă, fără limită de număr
    const subQSection = text.match(/### Analiză子问题\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
      const lines = subQSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.subQueries = lines.map(l => l.replace(/^\d+\.\s*/, '').trim()).filter(Boolean)
    }
    
    // Extrage faptele cheie - extragere completă, fără limită de număr
    const factsSection = text.match(/### 【关Cheie事实】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (factsSection) {
    ...
    // Extrage entitățile core - extragere completă, incluzând rezumate și număr de fapte relevante
    const entitySection = text.match(/### 【核心Entitate】\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
      const entityText = entitySection[1]
      // Împarte blocurile de entități după "- **"
    ...
    // Extrage lanțurile de relații - extragere completă, fără limită de număr
    const relSection = text.match(/### 【Relație链】\n([\s\S]*?)(?=\n###|$)/)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    // Extrage interogarea
    const queryMatch = text.match(/Interogare:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Extrage statisticile
    const nodesMatch = text.match(/总Nod数:\s*(\d+)/)
    const edgesMatch = text.match(/总边数:\s*(\d+)/)
    const activeMatch = text.match(/când前有效事实:\s*(\d+)/)
    ...
    // Extrage faptele valide curente - extragere completă, fără limită de număr
    const activeSection = text.match(/### 【când前有效事实】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
      const lines = activeSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.activeFacts = lines.map(l => {
        // Elimină numărul și ghilimelele
        const factText = l.replace(/^\d+\.\s*/, '').replace(/^"|"$/g, '').trim()
        return factText
      }).filter(Boolean)
    }
    
    // Extrage faptele istorice/expirate - extragere completă, fără limită de număr
    ...
    // Extrage entitățile implicate - extragere completă, fără limită de număr
    const entitySection = text.match(/### 【涉及Entitate】\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    // Extrage tema interviului
    const topicMatch = text.match(/\*\*采访主题:\*\*\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()
    
    // Extrage numărul de persoane intervievate (ex: "5 / 9 agenți simulați")
    ...
    // Extrage motivul selecției subiecților interviului
    const reasonMatch = text.match(/### 采访ObiectSelectare理由\n([\s\S]*?)(?=\n---\n|\n### 采访实录)/)
    if (reasonMatch) {
      result.selectionReason = reasonMatch[1].trim()
    }
    
    // Analizează motivul selecției pentru fiecare persoană
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Format 1: Număr. **Nume (index=X)**: Motiv
        // Exemplu: 1. **Alumni_345 (index=1)**: Ca absolvent al universității...
        ...
        // Format 2: - Selectează Nume (index X): Motiv
        // Exemplu: - Selectează Părinte_601 (index 0): Ca reprezentant al grupului de părinți...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*Selectare([^（(]+)(?:[（(]index\s*=?\s*\d+[)）])?[：:]\s*(.*)/)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        // Format 3: - **Nume (index X)**: Motiv
        // Exemplu: - **Părinte_601 (index 0)**: Ca reprezentant al grupului de părinți...
        if (!headerMatch) {
        ...
        if (name) {
          // Salvează motivul persoanei anterioare
          if (currentName && currentReason.length > 0) {
            reasons[currentName] = currentReason.join(' ').trim()
          }
          // Începe persoana nouă
          currentName = name
          currentReason = reasonStart ? [reasonStart.trim()] : []
        } else if (currentName && line.trim() && !line.match(/^未选|^综|^最终Selectare/)) {
          // Continuarea motivului (exclude paragraful final de rezumat)
          currentReason.push(line.trim())
        }
      }

      // Salvează motivul ultimei persoane
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    // Extrage fiecare înregistrare de interviu
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extrage răspunsul - separat pe Twitter și Reddit
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*关Cheie引言|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
        // Separa răspunsurile Twitter și Reddit
        const twitterMatch = answerText.match(/【TwitterPlatformă回答】\n?([\s\S]*?)(?=【RedditPlatformă回答】|$)/)
        const redditMatch = answerText.match(/【RedditPlatformă回答】\n?([\s\S]*?)$/)
        ...
        // Logică de fallback pentru platforme (compatibilitate cu formate vechi: doar o platformă marcată)
        if (!twitterMatch && redditMatch) {
          // Doar răspuns Reddit, copiază ca afișare implicită doar dacă nu este text placeholder
          if (interview.redditAnswer && interview.redditAnswer !== '(Platforma nu a primit răspuns)') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== '(Platforma nu a primit răspuns)') {
            interview.redditAnswer = interview.twitterAnswer
          }
        } else if (!twitterMatch && !redditMatch) {
          // Fără marcaje de platformă (format foarte vechi), folosește întregul text ca răspuns
          interview.twitterAnswer = answerText
        }
      }
      
      // Extrage citatele cheie (compatibil cu multiple formate de ghilimele)
      const quotesMatch = block.match(/\*\*关Cheie引言:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // Potrivire prioritară pentru formatul > "text"
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // Fallback: potrivește > "text" sau > \u201Ctext\u201D (ghilimele chinezești)
        if (!quoteMatches) {
    ...
    // Extrage rezumatul interviului
    const summaryMatch = text.match(/### 采访摘要și核心Opinie\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    // Extrage interogarea de căutare
    const queryMatch = text.match(/搜索Interogare:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Extrage numărul de rezultate
    const countMatch = text.match(/找la\s*(\d+)\s*条/)
    if (countMatch) result.count = parseInt(countMatch[1])
    
    // Extrage faptele relevante - extragere completă, fără limită de număr
    ...
    // Încearcă să extragă informațiile despre muchii (dacă există)
    ...
    // Încearcă să extragă informațiile despre noduri (dacă există)
    const nodesSection = text.match(/### 相关Nod:\n([\s\S]*?)(?=\n###|$)/)
    if (nodesSection) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-header' }, [
        ...
        props.result.simulationRequirement && h('div', { class: 'header-scenario' }, [
          h('span', { class: 'scenario-label' }, 'Scenariu predicție: '),
      ...
      h('div', { class: 'insight-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Memorii cheie curente (${props.result.facts.length})`)
        ]),
        h('button', {
          class: ['insight-tab', { active: activeTab.value === 'entities' }],
          onClick: () => { activeTab.value = 'entities' }
        }, [
          h('span', { class: 'tab-label' }, `Entități core (${props.result.entities.length})`)
        ]),
        h('button', {
          class: ['insight-tab', { active: activeTab.value === 'relations' }],
          onClick: () => { activeTab.value = 'relations' }
        }, [
          h('span', { class: 'tab-label' }, `Lanțuri de relații (${props.result.relations.length})`)
        ]),
        props.result.subQueries.length > 0 && h('button', {
          ...
          h('span', { class: 'tab-label' }, `Sub-întrebări (${props.result.subQueries.length})`)
        ])
      ]),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Memorii valide curente'),
          ...
          ) : h('div', { class: 'empty-state' }, 'Nu există memorii valide curente'),
          props.result.activeFacts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedActive.value = !expandedActive.value }
          }, expandedActive.value ? `Restrânge ▲` : `Expandează toate ${props.result.activeFacts.length} ▼`)
        ]),
          ...
          }, expandedFacts.value ? `Restrânge ▲` : `Expandează toate ${props.result.facts.length} ▼`)
        ]),

        // Entities Tab
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Entități core'),
          ...
          h('div', { class: 'entities-grid' },
            (expandedEntities.value ? props.result.entities : props.result.entities.slice(0, 12)).map((entity, i) => 
              h('div', { class: 'entity-tag', key: i, title: entity.summary || '' }, [
                ...
                entity.relatedFactsCount > 0 && h('span', { class: 'entity-fact-count' }, `${entity.relatedFactsCount} intrări`)
              ])
            )
          ),
          ...
          }, expandedEntities.value ? `Restrânge ▲` : `Expandează toate ${props.result.entities.length} ▼`)
        ]),
        
        // Relations Tab
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Lanțuri de relații'),
          ...
          }, expandedRelations.value ? `Restrânge ▲` : `Expandează toate ${props.result.relations.length} ▼`)
        ]),
        
        // Sub-queries Tab
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Sub-întrebări generate din analiza interogării de drift'),
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, 'Nu există memorii cheie curente'),
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'Nu există entități core'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'Nu există lanțuri de relații')
      ])
    ])
  }
}
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Memorii valide curente (${props.result.activeFacts.length})`)
        ]),
        h('button', {
          class: ['panorama-tab', { active: activeTab.value === 'historical' }],
          onClick: () => { activeTab.value = 'historical' }
        }, [
          h('span', { class: 'tab-label' }, `Memorii istorice (${props.result.historicalFacts.length})`)
        ]),
        h('button', {
          class: ['panorama-tab', { active: activeTab.value === 'entities' }],
          onClick: () => { activeTab.value = 'entities' }
        }, [
          h('span', { class: 'tab-label' }, `Entități implicate (${props.result.entities.length})`)
        ])
      ]),
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Memorii active curente'),
            h('span', { class: 'panel-count' }, `Total ${props.result.activeFacts.length} intrări`)
          ]),
          props.result.activeFacts.length > 0 ? h('div', { class: 'facts-list' },
            (expandedActive.value ? props.result.activeFacts : props.result.activeFacts.slice(0, INITIAL_SHOW_COUNT)).map((fact, i) => 
              h('div', { class: 'fact-item active', key: i }, [
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Memorii istorice'),
            ...
            (expandedHistorical.value ? props.result.historicalFacts : props.result.historicalFacts.slice(0, INITIAL_SHOW_COUNT)).map((fact, i) => 
              h('div', { class: 'fact-item historical', key: i }, [
                ...
                h('div', { class: 'fact-content' }, [
                  // Încearcă să extragă informațiile de timp [time - time]
                  (() => {
          ...
          ) : h('div', { class: 'empty-state' }, 'Nu există memorii istorice'),
          props.result.historicalFacts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedHistorical.value = !expandedHistorical.value }
          }, expandedHistorical.value ? `Restrânge ▲` : `Expandează toate ${props.result.historicalFacts.length} ▼`)
        ]),
        
        // Entities Tab
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Entități implicate'),
            h('span', { class: 'panel-count' }, `Total ${props.result.entities.length}`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'Nu există entități implicate'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Menține starea selecției platformei pentru fiecare pereche întrebare-răspuns
    const platformTabs = reactive({}) // { 'agentIdx-qIdx': 'twitter' | 'reddit' }
    
    // Obține selecția curentă a platformei pentru o întrebare
    const getPlatformTab = (agentIdx, qIdx) => {
      const key = `${agentIdx}-${qIdx}`
      return platformTabs[key] || 'twitter'
    }
    
    // Setează selecția platformei pentru o întrebare
    ...
    // Verifică dacă este text placeholder pentru platformă
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '（该Platformă未获得Răspuns）' || t === '(该Platformă未获得Răspuns)' || t === '[无Răspuns]' || t === '(Platforma nu a primit răspuns)' || t === '[Fără răspuns]'
    }

    // Încearcă să împartă răspunsul după numerele întrebărilor
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Suportă două formate de numerotare:
      // 1. "问题X：" sau "问题X:" (format chinezesc, format nou backend)
      // 2. "1. " sau "\n1. " (număr+punct, compatibilitate format vechi)
      let matches = []
      let match

      // Încearcă mai întâi formatul "问题X："
      const cnPattern = /(?:^|[\r\n]+)问题(\d+)[：:]\s*/g
      ...
      // Dacă nu s-a potrivit, folosește formatul "număr."
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // Dacă nu s-au găsit numere sau s-a găsit doar unul, returnează întregul text
      if (matches.length <= 1) {
      ...
      // Extrage fiecare parte după numere
      const parts = []
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Împărțire reușită și index valid
      if (answers.length > 1 && qIdx < answers.length) {
        return answers[qIdx] || ''
      }

      // Împărțire eșuată: prima întrebare primește răspunsul complet, restul gol
      return qIdx === 0 ? answer : ''
    }
    
    // Verifică dacă o întrebare are răspunsuri pe ambele platforme (filtrează textele placeholder)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Ambele platforme au răspunsuri reale (non-placeholder) și diferite
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Butoane de comutare platformă duală (afișate doar când există răspunsuri reale pe ambele platforme)
                  ...
                  // Buton Expandare/Restrângere (nu se afișează pentru texte placeholder)
    ...
    return () => h('div', { class: 'quick-search-display' }, [
      ...
      h('div', { class: 'quicksearch-header' }, [
        ...
        props.result.query && h('div', { class: 'header-query' }, [
          h('span', { class: 'query-label' }, 'Căutare: '),
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        ((!showTabs.value) || activeTab.value === 'facts') && h('div', { class: 'facts-panel' }, [
          !showTabs.value && h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Rezultate căutare'),
          ...
          ) : h('div', { class: 'empty-state' }, 'Nu s-au găsit rezultate'),
          props.result.facts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedFacts.value = !expandedFacts.value }
          }, expandedFacts.value ? `Restrânge ▲` : `Expandează toate ${props.result.facts.length} ▼`)
        ...
        activeTab.value === 'edges' && hasEdges.value && h('div', { class: 'edges-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Relații relevante'),
        ...
        activeTab.value === 'nodes' && hasNodes.value && h('div', { class: 'nodes-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Noduri relevante'),
...
// Pasul activ curent (folosit pentru afișare în partea de sus)
const activeStep = computed(() => {
  const steps = workflowSteps.value
  // Găsește pasul activ curent
  const active = steps.find(s => s.status === 'active')
  if (active) return active

  // Dacă nu există activ, returnează ultimul pas finalizat
  const doneSteps = steps.filter(s => s.status === 'done')
  if (doneSteps.length > 0) return doneSteps[doneSteps.length - 1]

  // Altfel returnează primul pas
  return steps[0] || { noLabel: '--', title: 'Așteaptă începerea', status: 'todo', meta: '' }
})
...
const renderMarkdown = (content) => {
  ...
  // Elimină titlul secundar de la început (## xxx), deoarece titlul capitolului este afișat în exterior
  let processedContent = content.replace(/^##\s+.+\n+/, '')

  // Procesează blocurile de cod
  let html = processedContent.replace(/```(\w*)\n([\s\S]*?)```/g, '<pre class="code-block"><code>$2</code></pre>')

  // Procesează codul inline
  html = html.replace(/`([^`]+)`/g, '<code class="inline-code">$1</code>')

  // Procesează titlurile
  html = html.replace(/^#### (.+)$/gm, '<h5 class="md-h5">$1</h5>')
  html = html.replace(/^### (.+)$/gm, '<h4 class="md-h4">$1</h4>')
  html = html.replace(/^## (.+)$/gm, '<h3 class="md-h3">$1</h3>')
  html = html.replace(/^# (.+)$/gm, '<h2 class="md-h2">$1</h2>')

  // Procesează blocurile de citat
  html = html.replace(/^> (.+)$/gm, '<blockquote class="md-quote">$1</blockquote>')

  // Procesează listele - suportă liste imbricate
  ...
  // Înwrap liste neordonate
  html = html.replace(/(<li class="md-li"[^>]*>.*?<\/li>\s*)+/g, '<ul class="md-ul">$&</ul>')
  // Înwrap liste ordonate
  html = html.replace(/(<li class="md-oli"[^>]*>.*?<\/li>\s*)+/g, '<ol class="md-ol">$&</ol>')

  // Curăță spațiile albe dintre elementele listei
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // Curăță spațiile albe după tagul de început al listei
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Curăță spațiile albe înainte de tagul de sfârșit al listei
  html = html.replace(/\s+<\/ul>/g, '</ul>')
  html = html.replace(/\s+<\/ol>/g, '</ol>')

  // Procesează bold și italic
  html = html.replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>')
  html = html.replace(/\*(.+?)\*/g, '<em>$1</em>')
  html = html.replace(/_(.+?)_/g, '<em>$1</em>')

  // Procesează liniile de separare
  html = html.replace(/^---$/gm, '<hr class="md-hr">')

  // Procesează liniile noi - rândurile goale devin separatoare de paragraf, liniile simple devin <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Curăță paragrafele goale
  html = html.replace(/<p class="md-p"><\/p>/g, '')
  html = html.replace(/<p class="md-p">(<h[2-5])/g, '$1')
  html = html.replace(/(<\/h[2-5]>)<\/p>/g, '$1')
  html = html.replace(/<p class="md-p">(<ul|<ol|<blockquote|<pre|<hr)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>|<\/pre>)<\/p>/g, '$1')
  // Curăță tagurile <br> înainte și după elementele de bloc
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // Curăță cazul <p><br> urmat de elemente de bloc (cauzat de linii goale în exces)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Curăță tagurile <br> consecutive
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Curăță <br> înainte de tagul de început al paragrafului după elemente de bloc
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Corectează numerotarea listelor ordonate necontinue: când <ol> individuale sunt separate de conținut paragraf, păstrează incrementarea
...
const getLogLevelClass = (log) => {
  if (log.includes('ERROR') || log.includes('Eroare') || log.includes('EROARE')) return 'error'
  if (log.includes('WARNING') || log.includes('Avertisment') || log.includes('AVERTISMENT')) return 'warning'
  // INFO folosește culoarea implicită, nu este marcat ca success
  return ''
}
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          // section_complete - Capitol generat complet
          if (log.action === 'section_complete') {
            if (log.details?.content) {
              generatedSections.value[log.section_index] = log.details.content
              // Extinde automat capitolul generat recent
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // Asigură eliminarea stării de încărcare
            emit('update-status', 'completed')
            stopPolling()
            // Logica de scroll este gestionată uniform în nextTick după terminarea buclei
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // Dacă sarcina este finalizată, derulează la început; altfel, derulează la sfârșit pentru a urmări cele mai recente loguri
            if (isComplete.value) {
...
// Extrage conținutul răspunsului final - extrage conținutul capitolului din răspunsul LLM
const extractFinalContent = (response) => {
  if (!response) return null

  // Încearcă să extragă conținutul din tagul <final_answer>
  ...
  // Încearcă să găsească conținutul după Final Answer: (suportă multiple formate)
  // Format 1: Final Answer:\n\nconținut
  // Format 2: Final Answer: conținut
  const finalAnswerMatch = response.match(/Final\s*Answer:\s*\n*([\s\S]*)$/i)
  if (finalAnswerMatch) {
    return finalAnswerMatch[1].trim()
  }

  // Încearcă să găsească conținutul după 最终答案: sau Răspuns final:
  const chineseFinalMatch = response.match(/最终答案[:：]\s*\n*([\s\S]*)$/i)
  if (chineseFinalMatch) {
    return chineseFinalMatch[1].trim()
  }
  
  const romanianFinalMatch = response.match(/Răspuns final[:：]\s*\n*([\s\S]*)$/i)
  if (romanianFinalMatch) {
    return romanianFinalMatch[1].trim()
  }

  // Dacă începe cu ## sau # sau >, poate fi conținut markdown direct
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }

  // Dacă conținutul este lung și conține format markdown, încearcă să elimine procesul de gândire și să returneze restul
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Elimină procesul de gândire care începe cu Thought:
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* Gri închis, nu se schimbă cu starea */
  font-weight: 500;
}
...
/* Console Logs - Sincronizat cu Step3Simulation.vue */
.console-logs {
  background: #000;
  color: #DDD;
  padding: 16px;

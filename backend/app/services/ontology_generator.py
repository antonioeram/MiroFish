"""
本体生成服务
接口1：分析文本内容，生成适合社会模拟的实体和Tipuri relații定义
"""
...
# 本体生成的系统提示词
ONTOLOGY_SYSTEM_PROMPT = """你是一个专业的知识图谱本体设计专家。你的任务是分析给定的文本内容和模拟需求，设计适合**社交媒体舆论模拟**的Tipuri entități和Tipuri relații。
...
```json
{
    "entity_types": [
        {
            "name": "Tipuri entități名称（英文，PascalCase）",
    ...
    "edge_types": [
        {
            "name": "Tipuri relații名称（英文，UPPER_SNAKE_CASE）",
            "description": "简短描述（英文，不超过100字符）",
            "source_targets": [
...
- 数量：6-10个
- 关系应该反映社媒互动中的真实联系
- 确保关系的 source_targets 涵盖你定义的Tipuri entități

### 3. 属性设计

- 每个Tipuri entități1-3个关键属性
...
"""
        ...
        message += """
请根据以上内容，设计适合社会舆论模拟的Tipuri entități和Tipuri relații。

**必须遵守的规则**：
1. 必须正好输出10个Tipuri entități
2. 最后2个必须是兜底类型：Person（个人兜底）和 Organization（组织兜底）
3. 前8个是根据文本内容设计的具体类型
4. 所有Tipuri entități必须是现实中可以发声的主体，不能是抽象概念
...
"""
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Zep API 限制：最多 10 个自定义Tipuri entități，最多 10 个自定义边类型
        MAX_ENTITY_TYPES = 10
        MAX_EDGE_TYPES = 10
